...
export const ProjectItem = ({ title, description, imageUrl, link, inProgress }: ProjectProps) => {
  return (
    <div className="flex w-full max-w-5xl flex-col gap-10 border-x border-b border-dashed px-8 py-14 last-of-type:border-b-0 sm:px-16 md:flex-row">
      ...
      <div className="bg-muted ring-muted-foreground/10 dark:ring-muted-foreground/30 rounded-3xl p-2 shadow-lg ring-1 backdrop-blur-md transition-all duration-500 hover:scale-[1.02] md:w-1/2">
        <Image
          src={imageUrl}
          width={1920}
          height={1080}
